...
sub open {
	...
	*$self->{CacheRemain} = 0;
	*$self->{Reader}->read(my $buffer, 9) || die("Can't read compression header");
	(*$self->{HdrCrc}, *$self->{StoredSize}, *$self->{Compressed}) = unpack('(L2c)<', $buffer);
	my $crc = Digest->new('CRC-32');
	$crc->add(substr($buffer, 4, 5));
	($crc->digest() == *$self->{HdrCrc}) || die("Invalid CRC in compression header");
...
sub refill {
	...
	# Read CRC
	*$self->{Reader}->read(my $buffer, 4) || die("Can't read block CRC");
	# Update counters
	...
	*$self->{Reader}->read(*$self->{Cache}, $length) || die("Can't read data block");
	# Update counters
	...
	*$self->{StoredRemain} -= $length;
	...
	my $crc = Digest->new('CRC-32');
	$crc->add(*$self->{Cache});
	($crc->digest() == $blockcrc) || die("Invalid CRC in compressed data");
