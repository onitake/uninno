...
sub TSetupComponentEntry {
	...
	$ret = {
		...
		Options => $self->ReadSet([ 'coFixed', 'coRestart', 'coDisableNoUninstallWarning' ]),
		Size => $self->ReadLongInt(),
	};
	return $ret;
}
...
sub TSetupFileEntry {
	...
	$ret = {
		...
		Options => $self->ReadSet([ 'foConfirmOverwrite', 'foUninsNeverUninstall', 'foRestartReplace', 'foDeleteAfterInstall', 'foRegisterServer', 'foRegisterTypeLib', 'foSharedFile', 'foCompareTimeStamp', 'foFontIsntTrueType', 'foSkipIfSourceDoesntExist', 'foOverwriteReadOnly', 'foOverwriteSameVersion', 'foCustomDestName', 'foOnlyIfDestFileExists', 'foNoRegError', 'foUninsRestartDelete', 'foOnlyIfDoesntExist', 'foIgnoreVersion', 'foPromptIfOlder' ]),
...
sub TSetupFileLocationEntry {
	...
	$ret = {
		...
		Flags => $self->ReadSet([ 'foVersionInfoValid', 'foVersionInfoNotValid', 'foBzipped' ]),
	};
	return $ret;
}
1;
