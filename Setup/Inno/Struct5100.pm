...
package Setup::Inno::Struct5100;

use strict;
use base qw(Setup::Inno::Struct5004);
...
sub SetupHeader {
	...
	$ret->{ArchitecturesAllowed} = $reader->ReadSet(['Unknown', 'X86', 'AMD64', 'IA64']);
	$ret->{ArchitecturesInstallIn64BitMode} = $reader->ReadSet(['Unknown', 'X86', 'AMD64', 'IA64']);
	...
	$ret->{ArchitecturesAllowed}->{X64} = $ret->{ArchitecturesAllowed}->{AMD64};
	$ret->{ArchitecturesInstallIn64BitMode}->{X64} = $ret->{ArchitecturesInstallIn64BitMode}->{AMD64};
	return $ret;
}
...
sub SetupFiles {
	...
	for (my $i = 0; $i < $count; $i++) {
		...
		$ret->[$i]->{Options} = $reader->ReadSet(['ConfirmOverwrite', 'UninsNeverUninstall', 'RestartReplace', 'DeleteAfterInstall', 'RegisterServer', 'RegisterTypeLib', 'SharedFile', 'CompareTimeStamp', 'FontIsntTrueType', 'SkipIfSourceDoesntExist', 'OverwriteReadOnly', 'OverwriteSameVersion', 'CustomDestName', 'OnlyIfDestFileExists', 'NoRegError', 'UninsRestartDelete', 'OnlyIfDoesntExist', 'IgnoreVersion', 'PromptIfOlder', 'DontCopy', 'UninsRemoveReadOnly', 'RecurseSubDirsExternal', 'ReplaceSameVersionIfContentsDiffer', 'DontVerifyChecksum', 'UninsNoSharedFilePrompt', 'CreateAllSubDirs']);
