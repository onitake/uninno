package Setup::Inno::Struct6102u;
use strict;
...
sub TSetupHeader {
	...
	$ret = {
		...
		SetupMutex => $self->ReadString(2),
		ChangesEnvironment => $self->ReadString(2),
		ChangesAssociations => $self->ReadString(2),
		...
		WizardStyle => $self->ReadEnum([ 'wsClassic', 'wsModern' ]),
		WizardSizePercentX => $self->ReadInteger(),
		WizardSizePercentY => $self->ReadInteger(),
		...
		PrivilegesRequiredOverridesAllowed => $self->ReadSet([ 'proCommandLine', 'proDialog' ]),
...
sub TSetupIconEntry {
	...
	$ret = {
		...
		AppUserModelToastActivatorCLSID => $self->TGUID(),
		MinVersion => {
			WinVersion => $self->ReadCardinal(),
			NTVersion => $self->ReadCardinal(),
			NTServicePack => $self->ReadWord(),
		...
		Options => $self->ReadSet([ 'ioUninsNeverUninstall', 'ioCreateOnlyIfFileExists', 'ioUseAppPaths', 'ioFolderShortcut', 'ioExcludeFromShowInNewInstall', 'ioPreventPinning', 'ioHasAppUserModelToastActivatorCLSID' ]),
	};
	return $ret;
}
sub TSetupIniEntry {
...
sub TSetupRunEntry {
	...
	$ret = {
		...
		Options => $self->ReadSet([ 'roShellExec', 'roSkipIfDoesntExist', 'roPostInstall', 'roUnchecked', 'roSkipIfSilent', 'roSkipIfNotSilent', 'roHideWizard', 'roRun32Bit', 'roRun64Bit', 'roRunAsOriginalUser', 'roDontLogParameters' ]),
	};
	return $ret;
}
